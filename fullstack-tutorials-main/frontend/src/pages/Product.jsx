...
const Product = () => {
  ...
  const { products, currency, addToCart, logoAndIcons } = useContext(ShopContext);
  const [productData, setProductData] = useState(null);
  const [size, setSize] = useState("");
  const [image, setImage] = useState("");

  // Filter star_logo and star_dull icons
  const logoData = (logoAndIcons && Array.isArray(logoAndIcons.data))
    ? logoAndIcons.data
  ...
  const fetchProductData = async () => {
    if (products && Array.isArray(products.data)) {
      const foundProduct = products.data.find(item => item._id === productId);

      if (foundProduct) {
        setProductData(foundProduct);
        setImage(foundProduct.image && foundProduct.image.length > 0 ? foundProduct.image[0] : 'fallback_image_url');
      } else {
      ...
      <div className='flex gap-12 sm:gap-12 flex-col sm:flex-row'>  {/* -------- Product Row ---------- */}
        ...
        <div className='flex-1'>
          ...
          <div className='flex items-center gap-1 mt-2'>
            <img className='w-3.5' src={logoData[0]} alt="" />
            <img className='w-3.5' src={logoData[0]} alt="" />
            <img className='w-3.5' src={logoData[0]} alt="" />
            <img className='w-3.5' src={logoData[0]} alt="" />
            <img className='w-3.5' src={logoData[1]} alt="" />
